...
Complexity Levels
    ...
    Deploy a web API without DB access and demonstrate that the CI/CD pipeline works and the API is reachable via e.g. REST Client.
    Add a simple Angular frontend.

    Step 1: Without API access
    Step 2: With API access

    Add DB access to the API
    Secure DB connection management (e.g. Managed Identity, Key Vault, etc.)
    Automate the creation of the Azure infrastructure (e.g. using Bicep, Open Tofu, etc.)
